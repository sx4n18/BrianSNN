...
neuron_groups['e'] = b.NeuronGroup(n_e*len(population_names), neuron_eqs_e, threshold= v_thresh_e_str, refractory= refrac_e, reset= scr_e,
                                   )
neuron_groups['i'] = b.NeuronGroup(n_i*len(population_names), neuron_eqs_i, threshold='v>v_thresh_i', refractory= refrac_i, reset='v= v_reset_i',
                                   )
...
for name in population_names:
    ...
    Ae = neuron_groups['e'] #.subgroup(n_e)  this function has been removed from brian2 library
    Ai = neuron_groups['i'] #.subgroup(n_i)
    ...
    for conn_type in recurrent_conn_names:
        ...
        #-----------------------updated by Shouyu------------------------
        connections[connName] = b.Synapses(neuron_groups[connName[1]], neuron_groups[connName[3]], 'w : 1', on_pre= get_SynFun(connName))
        connections[connName].connect(i=source,j=target)
        for m,n in zip(source,target):
            connections[connName].w[m, n] = weightMatrix[m, n]
    ...
    rate_monitors_Ae = b.PopulationRateMonitor(Ae)
    rate_monitors_Ai = b.PopulationRateMonitor(Ai)
    spike_counters[name+'e'] = b.SpikeMonitor(Ae,record=True)
    
    if record_spikes:
        spike_monitors_Ae = b.SpikeMonitor(Ae,record=True)
        spike_monitors_Ai = b.SpikeMonitor(Ai,record=True)
...
for i,name in enumerate(input_population_names):
    ...
    rate_monitors[name+'e'] = b.PopulationRateMonitor(Xe)
...
for name in input_connection_names:
    ...
    for connType in input_conn_names:
        ...
        print(np.count_nonzero(weightMatrix), 'non-zero values were detected for ' + connName)
        # -----------------------updated by Shouyu------------------------
        connections[connName] = b.Synapses(Xe, neuron_groups[connName[3]],'w :1', on_pre= get_SynFun(connName))
        #connections[connName].connect(input_groups[connName[0:2]], neuron_groups[connName[2:4]], weightMatrix, delay=delay[connType])
        connections[connName].connect(i=source, j=target)
        for m, n in zip(source, target):
            connections[connName].w[m, n] = weightMatrix[m, n]
        connections[connName].delay='rand()*10*ms'
    if ee_STDP_on:
...
#     print 'run number:', j+1, 'of', int(num_examples)
    ...
    current_spike_count = np.asarray(spike_monitors_Ae.count[:]) - previous_spike_count
    previous_spike_count = np.copy(spike_monitors_Ae.count[:])
    if np.sum(current_spike_count) < 5 :
        print('Inadequate input intensity, adjustment needed')
        input_intensity += 1
        if input_intensity>8:
            print('Current input pattern')
